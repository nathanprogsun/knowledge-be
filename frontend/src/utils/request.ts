...
// W3C traceparent per request: the backend's OTel FastAPI
// instrumentation extracts it, making the browser action the root
// context of the server trace. Random ids suffice — no SDK needed.
...
instance.interceptors.request.use(
  (config) => {
    ...
    // 但这条优化会被任何把 kb_tenant 写成激活空间的代码（OIDC
    ...
    if (!isEmbedAuth && !isEmbedPath) {
      const selectedTenantId = localStorage.getItem('kb_selected_tenant_id');
      if (selectedTenantId) {
      ...
      if (refreshToken) {
        try {
          ...
          if (response.success && response.data) {
            ...
            localStorage.setItem('kb_refresh_token', newRefreshToken);
