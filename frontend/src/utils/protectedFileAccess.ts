...
const accessState: ProtectedFileAccessState = (() => {
  ...
  const scope = window as typeof window & {
    __kbProtectedFileAccessV1__?: ProtectedFileAccessState;
  };
  scope.__kbProtectedFileAccessV1__ ||= fresh();
  return scope.__kbProtectedFileAccessV1__;
})();
...
function tenantRequestHeaders(): Record<string, string> {
  ...
  try {
    const token = (localStorage.getItem('kb_token') || '').trim();
    if (token) {
      headers['Authorization'] = `Bearer ${token}`;
    }

    const selectedTenantId = (localStorage.getItem('kb_selected_tenant_id') || '').trim();
    if (selectedTenantId) {
      ...
      // active tenant into kb_tenant, leaving authenticated file
