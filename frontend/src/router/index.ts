...
import { autoSetup, getCurrentUser, tenantOwnerId, userInfoFromApi } from '@/api/auth'

/** Lite /桌面 WebView 硬刷新时可能只打开 `/`，用 session 记住上次页面以便恢复 */
const LITE_LAST_PATH_KEY = 'kb_lite_last_path'
const AUTO_SETUP_FAILED_KEY = 'kb_auto_setup_failed'
...
function isLiteEdition(authStore: ReturnType<typeof useAuthStore>) {
  return authStore.isLiteMode || localStorage.getItem('kb_lite_mode') === 'true'
}
  ...
  try {
    ...
    if (tenant) {
      authStore.setTenant({
        id: String(tenant.id) || '',
        name: tenant.name || '',
        owner_id: tenantOwnerId(tenant, user.id || ''),
