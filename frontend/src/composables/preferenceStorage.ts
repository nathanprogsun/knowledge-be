 ...
 *
 * Storage layout: kb_${userId}_${suffix}, where userId is the active
 ...
 *   - Legacy un-namespaced keys (kb_${suffix}) from earlier branch
...
export function safeSetItem(key: string, value: string): void {
  try {
    ...
    console.warn(`[Knowledge Base] failed to persist preference "${key}":`, err)
  }
}
...
export function userKey(suffix: string): string {
  return `kb_${readUserId()}_${suffix}`
}
...
export function migratePreferencesIntoUser(): void {
  ...
  for (const suffix of PREFERENCE_SUFFIXES) {
    const target = `kb_${userId}_${suffix}`
    const targetExists = safeGetItem(target) !== null

    const anonKey = `kb_anon_${suffix}`
    const legacyKey = `kb_${suffix}`
