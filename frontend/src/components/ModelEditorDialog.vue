<template>
  <SettingDrawer :visible="dialogVisible" :title="isEdit ? $t('model.editor.editTitle') : $t('model.editor.addTitle')"
    ...
    :confirm-disabled="formData.provider === 'cloud' && wkcCredentialState !== 'configured'"
    ...
    <template v-if="formData.source === 'remote'" #footer-left>
      <t-button variant="outline" @click="checkRemoteAPI" :loading="checking"
        :disabled="!formData.modelName || (!formData.baseUrl && formData.provider !== 'cloud') || (formData.provider === 'cloud' && wkcCredentialState !== 'configured')">
        <template #icon>
    ...
    <t-form ref="formRef" :data="formData" :rules="rules" layout="vertical">
      ...
      <template v-if="formData.source === 'remote'">
        <section class="setting-drawer__section">
          ...
          <!-- Cloud 提示信息 -->
          <template v-if="formData.provider === 'cloud'">
            <!-- 凭证已配置 -->
            <div v-if="wkcCredentialState === 'configured'" class="cloud-hint cloud-hint--ok">
              <t-icon name="check-circle-filled" class="hint-icon hint-icon--ok" />
              <div>
                {{ $t('settings.cloud.modelHintConfigured') }}
                <a href="https://developers.weixin.qq.com/doc/aispeech/knowledge/atomic_capability/atomic_interface.html"
                  ...
                  {{ $t('settings.cloud.modelHintDocsLink') }}
            ...
            <!-- 未配置 / 失效 -->
            <div v-else-if="wkcCredentialState !== 'loading'" class="cloud-hint cloud-hint--warn">
              <t-icon name="error-circle-filled" class="hint-icon hint-icon--warn" />
              <div style="flex: 1;">
                <template v-if="wkcCredentialState === 'expired'">
                  {{ $t('settings.cloud.credentialExpired') }}
                </template>
                <template v-else>
                  {{ $t('settings.cloud.credentialUnconfigured') }}
                </template>
                <div style="margin-top: 8px;">
                  <t-button variant="text" size="small" @click="goToCloudSettings"
                    style="padding: 0; height: auto;">
                    <template #icon><t-icon name="jump" /></template>
                    {{ $t('settings.cloud.goToSettings') }}
                  </t-button>
                </div>
              </div>
            </div>
            ...
            <!-- 加载中 -->
            <div v-else class="cloud-hint">
              <t-icon name="loading" class="spinning hint-icon hint-icon--loading" />
              <span>{{ $t('settings.cloud.checkingStatus') }}</span>
            </div>
          </template>
          ...
          <div class="form-item">
            ...
            <t-input v-model="formData.modelName" :placeholder="getModelNamePlaceholder()"
              :disabled="formData.provider === 'cloud' && wkcCredentialState !== 'configured'" />
          </div>
          ...
          <div v-if="formData.provider !== 'cloud'" class="form-item">
...
const credentialFields = computed<CredentialFieldDef<ModelCredentialField>[]>(() => {
  ...
  if (formData.value.provider === 'cloud') {
...
const goToCloudSettings = async () => {
  emit('update:visible', false)
  if (uiStore.showSettingsModal) {
    uiStore.closeSettings()
    await nextTick()
  }
  uiStore.openSettings('cloud')
}
...
const checkRemoteAPI = async () => {
  if (!formData.value.modelName || (!formData.value.baseUrl && formData.value.provider !== 'cloud')) {
...
const handleConfirm = async () => {
  try {
    ...
    // 如果是 remote 类型且非 Cloud，必须填写 baseUrl
    if (formData.value.source === 'remote' && formData.value.provider !== 'cloud') {
