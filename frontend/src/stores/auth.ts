...
import { tenantOwnerId, userInfoFromApi } from '@/api/auth'
...
export const useAuthStore = defineStore('auth', () => {
  ...
  //
  // SECURITY: This value is read from localStorage (`kb_memberships`)
  ...
  const setTenant = (tenantData: TenantInfo | null) => {
    ...
    if (tenantData) {
      localStorage.setItem('kb_tenant', JSON.stringify(tenantData))
    } else {
      localStorage.removeItem('kb_tenant')
  ...
  const setToken = (tokenValue: string) => {
    ...
    localStorage.setItem('kb_token', tokenValue)
  }

  const setRefreshToken = (refreshTokenValue: string) => {
    refreshToken.value = refreshTokenValue
    localStorage.setItem('kb_refresh_token', refreshTokenValue)
  }

  const setKnowledgeBases = (kbList: KnowledgeBaseInfo[]) => {
    // 确保输入是数组
    knowledgeBases.value = Array.isArray(kbList) ? kbList : []
    localStorage.setItem('kb_knowledge_bases', JSON.stringify(knowledgeBases.value))
  }

  const setCurrentKnowledgeBase = (kb: KnowledgeBaseInfo | null) => {
    currentKnowledgeBase.value = kb
    if (kb) {
      localStorage.setItem('kb_current_kb', JSON.stringify(kb))
    } else {
      localStorage.removeItem('kb_current_kb')
    }
  }
  ...
  const clearTenantScopedClientState = () => {
    try {
      localStorage.removeItem('kb_last_chat_model_id')
      localStorage.removeItem('kb_current_kb')
      const raw = localStorage.getItem('kb_settings')
      if (raw) {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          localStorage.setItem('kb_settings', JSON.stringify(parsed))
        }
      }
    } catch (e) {
  ...
  const setSelectedTenant = (tenantId: number | null, tenantName: string | null = null) => {
    ...
    if (tenantId !== null) {
      localStorage.setItem('kb_selected_tenant_id', String(tenantId))
      if (tenantName) {
        localStorage.setItem('kb_selected_tenant_name', tenantName)
      }
    } else {
      localStorage.removeItem('kb_selected_tenant_id')
      localStorage.removeItem('kb_selected_tenant_name')
    }
    if (tenantChanged) {
  ...
  const setMemberships = (
    ...
    localStorage.setItem('kb_memberships', JSON.stringify(memberships.value))
  }
    ...
    try {
      ...
      if (tenantSnapshot) {
        setTenant({
          ...
          owner_id: tenantOwnerId(tenantSnapshot, u.id || ''),
  ...
  const setLiteMode = (value: boolean) => {
    ...
    if (value) {
      localStorage.setItem('kb_lite_mode', 'true')
    } else {
      localStorage.removeItem('kb_lite_mode')
    }
  }
  ...
  const logout = () => {
    ...
    localStorage.removeItem('kb_knowledge_bases')
    localStorage.removeItem('kb_current_kb')
    localStorage.removeItem('kb_selected_tenant_id')
    localStorage.removeItem('kb_selected_tenant_name')
    localStorage.removeItem('kb_memberships')
    localStorage.removeItem('kb_lite_mode')
    isLiteMode.value = false
    try {
      sessionStorage.removeItem('kb_lite_last_path')
    } catch {
      /* ignore */
    }
    reloadUserPreferences()
  ...
  const initFromStorage = () => {
    ...
    const storedTenant = localStorage.getItem('kb_tenant')
    const storedToken = localStorage.getItem('kb_token')
    const storedRefreshToken = localStorage.getItem('kb_refresh_token')
    const storedKnowledgeBases = localStorage.getItem('kb_knowledge_bases')
    const storedCurrentKb = localStorage.getItem('kb_current_kb')
    const storedSelectedTenantId = localStorage.getItem('kb_selected_tenant_id')
    const storedSelectedTenantName = localStorage.getItem('kb_selected_tenant_name')
    ...
    const storedMemberships = localStorage.getItem('kb_memberships')
    ...
    isLiteMode.value = localStorage.getItem('kb_lite_mode') === 'true'
  }
