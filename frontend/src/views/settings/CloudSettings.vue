<template>
  <div class="cloud-settings">
    ...
    <div v-if="credentialState === 'unconfigured'" class="credential-status unconfigured">
      ...
      <span>{{ $t('settings.cloud.unconfigured') }}</span>
    </div>
    ...
    <div v-else-if="credentialState === 'expired'" class="credential-warning">
      ...
      <div class="warning-text">
        <strong>{{ $t('settings.cloud.expired') }}</strong><br />
        {{ reinitReason || $t('settings.cloud.expiredDefault') }}
      </div>
    </div>
    ...
    <div v-else-if="credentialState === 'configured'" class="credential-status success">
      ...
      <span class="status-text">{{ $t('settings.cloud.configured') }}</span>
      <t-button
        v-if="!formExpanded"
        variant="outline"
        theme="default"
    ...
    <div v-if="formExpanded" class="settings-group">
      <div class="setting-row">
        <div class="setting-info">
          <label class="setting-label">{{ $t('settings.cloud.appIdLabel') }}</label>
          <p class="setting-desc">{{ $t('settings.cloud.appIdDesc') }}</p>
        </div>
        <div class="setting-control">
          <t-input
            v-model="form.appId"
            :placeholder="$t('settings.cloud.appIdPlaceholder')"
            autocomplete="off"
            style="width: 280px;"
          />
        </div>
      ...
      <div class="setting-row">
        <div class="setting-info">
          <label class="setting-label">{{ $t('settings.cloud.appSecretLabel') }}</label>
          <p class="setting-desc">{{ $t('settings.cloud.appSecretDesc') }}</p>
        </div>
        <div class="setting-control">
          <t-input
            v-model="form.appSecret"
            type="password"
            :placeholder="$t('settings.cloud.appSecretPlaceholder')"
      ...
      <div class="setting-row action-row">
        <div class="setting-info">
          <p class="setting-desc">{{ $t('settings.cloud.saveHint') }}</p>
        </div>
        <div class="setting-control">
          <t-button
            theme="primary"
    ...
    >
      <div class="models-section__header">
        <h3 class="models-section__title">{{ $t('settings.cloud.modelsSection.title') }}</h3>
        <p class="models-section__desc">
          {{
            credentialState === 'configured'
              ? $t('settings.cloud.modelsSection.descReady')
              : $t('settings.cloud.modelsSection.descPending')
          }}
        </p>
      </div>
        ...
        >
          ...
          <div class="model-row__action">
            <t-tag
              ...
              size="small"
            >
              {{ $t('settings.cloud.modelsSection.statusAdded') }}
            </t-tag>
            <t-popconfirm
              v-else-if="credentialState === 'configured'"
              :content="$t('settings.cloud.modelsSection.confirmAddOne', {
                type: kindLabel(kind),
                name: WKC_MODEL_NAME_BY_KIND[kind],
              })"
              :confirm-btn="{ content: $t('settings.cloud.modelsSection.addOne'), theme: 'primary' }"
            ...
            >
              <t-button
                ...
                @click.stop
              >
                {{ $t('settings.cloud.modelsSection.addOne') }}
              </t-button>
            </t-popconfirm>
            <span v-else class="model-row__pending">
              {{ $t('settings.cloud.modelsSection.statusPending') }}
            </span>
          </div>
        </div>
      </div>
      ...
      >
        <t-popconfirm
          :content="$t('settings.cloud.modelsSection.confirmAddAll', { count: missingKinds.length })"
          :confirm-btn="{
            content: $t('settings.cloud.modelsSection.addAllConfirm'),
            theme: 'primary',
          }"
          :cancel-btn="{ content: $t('common.cancel') }"
          placement="top"
        ...
        >
          ...
          >
            {{ $t('settings.cloud.modelsSection.addAllBtn', { count: missingKinds.length }) }}
          </t-button>
        </t-popconfirm>
      </div>
      ...
      >
        ...
        {{ $t('settings.cloud.modelsSection.allReady') }}
      </p>
    </section>

    <!-- 使用说明 -->
    <div class="usage-hint">
      <p class="hint-title">{{ $t('settings.cloud.usageTitle') }}</p>
      <p class="hint-text" v-html="$t('settings.cloud.usageSteps').replace(/\n/g, '<br />')" />
    </div>
  </div>
</template>
...
import { saveCloudCredentials, getCloudStatus, createModel, listModels } from '@/api/model'
...
const kindDisplayName = (kind: WkcModelKind) =>
  t(`settings.cloud.addModelsDisplayName.${kind}`)
...
const resolveEmbeddingDimension = async (): Promise<number> => {
  const result = await testEmbeddingModel({
    ...
    baseUrl: KB_CLOUD_BASE_URL,
    provider: KB_CLOUD_PROVIDER,
  })
  if (!result.available || !result.dimension) {
    throw new Error(result.message || t('settings.cloud.addModelsEmbeddingFailed'))
  }
...
const addModels = async (kinds: WkcModelKind[]) => {
  ...
  try {
    ...
    if (success > 0 && failed === 0) {
      MessagePlugin.success(t('settings.cloud.addModelsSuccess', { count: success }))
    } else if (success > 0) {
      MessagePlugin.warning(t('settings.cloud.addModelsPartial', { success, failed }))
    } else {
      MessagePlugin.error(t('settings.cloud.addModelsFailed'))
    }
  } finally {
...
const handleSave = async () => {
  if (!form.value.appId || !form.value.appSecret) {
    MessagePlugin.warning(t('settings.cloud.fillRequired'))
    return
  }
  saving.value = true
  try {
    ...
    MessagePlugin.success(t('settings.cloud.saveSuccess'))
    form.value.appId = ''
    ...
    MessagePlugin.error(err?.message || t('settings.cloud.saveFailed'))
  } finally {
    saving.value = false
  }
}
