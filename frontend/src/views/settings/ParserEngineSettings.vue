  ...
  <div class="parser-engine-settings">
    ...
    >
      ...
      <div v-if="currentEngine">
        ...
        <!--
          Section 2 — 状态信息（DocReader 连接 / Cloud 凭证）
          只有有内容时才渲染，避免空 section 空底部分隔线。
        -->
        <section
          v-if="currentEngine.Name === 'builtin' || currentEngine.Name === 'cloud'"
          class="setting-drawer__section"
        >
          ...
          <template v-if="currentEngine.Name === 'cloud'">
            ...
            <div v-else class="inline-alert inline-alert--warn">
              ...
              <span class="inline-alert__text">
                <span v-if="wkcState === 'expired'">{{ $t('settings.cloud.credentialExpired') }}</span>
                <span v-else>{{ $t('settings.cloud.unconfigured') }}</span>
              </span>
...
import { getCloudStatus } from '@/api/model'
...
const ENGINE_DOC_LINKS: Record<string, string> = {
  cloud: 'https://developers.weixin.qq.com/doc/aispeech/knowledge/atomic_capability/atomic_interface.html',
...
.engine-card--cloud .engine-card__badge {
...
// .form-item / .form-label / .form-desc / .cloud-hint / .api-test
...
// ---- Inline alert（替代之前的 .cloud-hint 卡片） ----
...
.parser-engine-drawer--cloud .setting-drawer__header-icon {
  background: rgba(7, 192, 95, 0.12);
  color: #07C05F;
}
