...
type KbDesktopWindow = Window & {
  __KB_API_BASE__?: string
  __KB_API_LAN_BASE__?: string
  go?: {
    main?: {
      App?: {
...
async function tryLoadWailsApiBaseURL() {
  const win = window as KbDesktopWindow
  for (let i = 0; i < 40; i++) {
    const injected = win.__KB_API_BASE__
...
async function tryLoadWailsLanHints(win: KbDesktopWindow) {
  const injectedLan = win.__KB_API_LAN_BASE__
...
function desktopPortBindingsAvailable(win: KbDesktopWindow) {
...
function desktopBindPublicBindingsAvailable(win: KbDesktopWindow) {
  const app = win.go?.main?.App
  return (
...
const onDesktopBindPublicChange = async (value: boolean) => {
  ...
  const fn = (window as KbDesktopWindow).go?.main?.App?.SetDesktopHTTPBindPublicSetting
...
const saveDesktopPort = async () => {
  ...
  const fn = (window as KbDesktopWindow).go?.main?.App?.SetDesktopHTTPPortSetting
  if (typeof fn !== 'function') return
  try {
