...
const applyAgentListData = (res: { data: CustomAgent[]; disabled_own_agent_ids: string[] }) => {
  ...
  // D-05 tracing: log the post-load snapshot so we can correlate with
  // the section-collapsed state when reproducing the '1 agent, hidden'
  // symptom in the browser.
  if (typeof window !== 'undefined') {
    const mineCount = agents.value.filter(a => !a.is_builtin && isMyAgent(a as AgentWithUI)).length
    console.debug('[agent-list] loaded', {
      total: agents.value.length,
      mineCount,
      collapsedAtLoad: [...collapsedAgentSections.value],
    })
  }
...
const isAgentSectionCollapsed = (key: AgentSectionKey) => {
  const result = collapsedAgentSections.value.has(key)
  // D-05 tracing: log every lookup so we can confirm the initial state.
  if (typeof window !== 'undefined') {
    console.debug('[agent-section] isCollapsed', { key, result, set: [...collapsedAgentSections.value] })
  }
  return result
}
const toggleAgentSection = (key: AgentSectionKey) => {
  ...
  // D-05 tracing: log every toggle so we can see who is collapsing 'mine'.
  if (typeof window !== 'undefined') {
    console.debug('[agent-section] toggle', { key, nowCollapsed: next.has(key), set: [...next] })
  }
}
