...
export interface InitializeCloudRequest {
  app_id: string
  app_secret: string
}

// 仅保存 Cloud 凭证，不自动创建模型
export function saveCloudCredentials(data: InitializeCloudRequest): Promise<{ success: boolean; message: string }> {
  return new Promise((resolve, reject) => {
    post('/api/v1/cloud/credentials', data)
      .then((response: any) => {
        if (response.success) {
          resolve(response)
        } else {
...
export function getCloudStatus(): Promise<CloudStatusResult> {
  return new Promise((resolve, reject) => {
    get('/api/v1/models/cloud/status')
      .then((response: any) => {
