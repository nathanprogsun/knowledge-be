...
export const EMBED_CHAT_SESSION_STORAGE_PREFIX = 'kb-embed-session:'

/** localStorage key prefix for anonymous embed visitor ids (per channel). */
export const EMBED_VISITOR_STORAGE_PREFIX = 'kb-embed-visitor:'
...
const EMBED_MSG_SOURCE = 'kb-embed'
const EMBED_HOST_SOURCE = 'kb-host'
...
export function buildWidgetSnippet(
  ...
  const attrs = [
    `src="${escapeHtmlAttr(`${base}/kb-widget.js`)}"`,
...
export const SECURE_TOKEN_ENDPOINT_PLACEHOLDER = 'https://your-backend.example.com/kb/embed-token'
...
export function buildSecureServerNodeExample(channelId: string, opts?: { baseUrl?: string }) {
  ...
  return [
    `// Node/Express — keep KB_PUBLISH_TOKEN only on the server (env var).`,
    `app.get('/kb/embed-token', async (req, res) => {`,
    ...
    `    method: 'POST',`,
    `    headers: {`,
    `      Authorization: 'Embed ' + process.env.KB_PUBLISH_TOKEN,`,
...
export function buildSecureServerGoExample(channelId: string, opts?: { baseUrl?: string }) {
  ...
  const exchangeUrl = `${base}/api/v1/embed/${channelId}/exchange`
  return [
    `// Go net/http — keep KB_PUBLISH_TOKEN only on the server (env var).`,
    ...
    `  req.Header.Set("Authorization", "Embed "+os.Getenv("KB_PUBLISH_TOKEN"))`,
