 ...
 *
 * Programmatic:
 *   Knowledge Base.init({ channel, token, position, primaryColor, title, baseUrl })
 *   Knowledge Base.open() | close() | toggle() | destroy()
 *   Knowledge Base.on('ready', fn) | off('ready', fn)
 *
 * Host context & actions (postMessage to iframe):
 *   Knowledge Base.setContext({ userId, page, ... })
 *     Inject visitor/page context merged into each chat query.
 *   Knowledge Base.openWithQuery('How do I reset my password?')
 *     Opens the panel (if closed) and sends the query when the iframe is ready.
 *   Knowledge Base.setLocale('en-US')
...
(function (global) {
  ...
  var HOST_SOURCE = 'kb-host';
  var EMBED_SOURCE = 'kb-embed';
  ...
  function createWidget(opts) {
    ...
    if (!baseUrl) {
      ...
      if (script && script.src) {
        baseUrl = script.src.replace(/\/kb-widget\.js.*$/, '');
      } else {
