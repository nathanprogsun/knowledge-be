...
async def test_list_all_orders_by_type_then_title(session: AsyncSession) -> None:
    ...
    await _insert_page(
        repo,
        _sample_page(tenant_id=tid, knowledge_base_id=kb, slug="concept/rag", title="RAG"),
    )
    await _insert_page(
        repo,
        _sample_page(tenant_id=tid, knowledge_base_id=kb, slug="entity/acme", title="Acme"),
...
async def test_count_by_type_and_count_orphans(session: AsyncSession) -> None:
    ...
    await _insert_page(repo, _sample_page(tenant_id=tid, knowledge_base_id=kb, slug="concept/b"))
    await _insert_page(
        repo,
        _sample_page(
            tenant_id=tid,
