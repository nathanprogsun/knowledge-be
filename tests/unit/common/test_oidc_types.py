"""Unit tests for ``src.common.oidc_types``.
...
def test_oidc_user_info_mapping_constructs_with_valid_fields() -> None:
    from src.common.oidc_types import OIDCUserInfoMapping
...
def test_oidc_standard_claims_accepts_typical_payload() -> None:
    from src.common.oidc_types import OIDCStandardClaims
...
def test_address_claim_accepts_typical_payload() -> None:
    from src.common.oidc_types import AddressClaim
...
def test_oidc_claims_dict_accepts_standard_claims() -> None:
    from src.common.oidc_types import OIDCClaimsDict, OIDCStandardClaims
...
def test_oidc_claims_dict_accepts_custom_claims() -> None:
    ...
    # Literal goes through dict[str, JsonValue] first: mypy otherwise
    # resolves a dict literal against the TypedDict union member and
    # rejects the non-standard keys.
    raw: dict[str, JsonValue] = {
        "tenant_id": "acme",
        "groups": ["admin", "dev"],
        "custom_number": 42,
    }
    custom: OIDCClaimsDict = raw
    # Index the plain-dict view: indexing the union directly makes mypy
    # check the TypedDict member, which rejects non-standard keys.
    custom_dict = dict(custom)
    assert custom_dict["tenant_id"] == "acme"
    assert custom_dict["groups"] == ["admin", "dev"]
