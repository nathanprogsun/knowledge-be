"""Tests for the knowledge-base share repository against the real applied schema.
...
async def test_create_and_get_by_id(session: AsyncSession) -> None:
    repo = KBShareRepository(session)
    share = _share()
    ...
    assert fetched.knowledge_base_id == share.knowledge_base_id
    assert fetched.permission == "viewer"


async def test_get_by_kb_and_org(session: AsyncSession) -> None:
    repo = KBShareRepository(session)
    share = _share()
    await repo.create(share)
    await session.commit()

    fetched = await repo.get_by_kb_and_org_or_none(
...
async def test_soft_deleted_share_can_be_recreated(session: AsyncSession) -> None:
    ...
    await session.commit()
    # A fresh share of the same (kb, org) pair gets a new id; the partial
    # unique index only guards live rows, so the pair is reusable.
    recreated = _share(
        knowledge_base_id=share.knowledge_base_id,
        organization_id=share.organization_id,
...
async def test_list_by_knowledge_base(session: AsyncSession) -> None:
    repo = KBShareRepository(session)
    kb_id = f"kb-{uuid.uuid4().hex[:12]}"
    await repo.create(_share(knowledge_base_id=kb_id))
    await repo.create(_share(knowledge_base_id=kb_id))
    await session.commit()

    shares = await repo.list_by_knowledge_base(kb_id)
...
async def test_count_by_knowledge_base(session: AsyncSession) -> None:
    ...
    count = await repo.count_by_knowledge_base(kb_id)

    assert count == 2


async def test_delete_by_knowledge_base(session: AsyncSession) -> None:
    repo = KBShareRepository(session)
    kb_id = f"kb-{uuid.uuid4().hex[:12]}"
    await repo.create(_share(knowledge_base_id=kb_id))
    await repo.create(_share(knowledge_base_id=kb_id))
    ...
    affected = await repo.delete_by_knowledge_base(knowledge_base_id=kb_id, now=_NOW)
    await session.commit()

    assert affected == 2
    assert await repo.list_by_knowledge_base(kb_id) == []
