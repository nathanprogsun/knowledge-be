"""Azure OpenAI embedding provider (azure_openai.go).

Implements text vectorization over the Azure OpenAI ``/embeddings``
endpoint. Azure is OpenAI-compatible in response shape but differs on the
wire: the deployment URL embeds the model name and an ``api-version``
query parameter, and authentication uses an ``api-key`` header instead of
``Authorization: Bearer``. ``truncate_prompt_tokens`` is accepted by the
constructor for signature symmetry but is not part of the Azure request
body (mirroring the upstream struct).
...
class AzureOpenAIEmbedRequest(BaseModel):
    """Azure ``/embeddings`` request body (``azureOpenAIEmbedRequest``)."""
...
class AzureOpenAIEmbedResponse(BaseModel):
    """``/embeddings`` response body (reuses the OpenAI response shape)."""
...
class AzureOpenAIEmbedder:
    """Azure OpenAI text vectorizer (``AzureOpenAIEmbedder``)."""
    ...
    async def batch_embed(self, ctx: base.Context, texts: list[str]) -> list[list[float]]:
        request = AzureOpenAIEmbedRequest(
        ...
        url = (
            f"{self._base_url}/openai/deployments/{self._model_name}/embeddings"
        ...
        if response.status_code != 200:
            ...
            raise AIProviderError(
                f"Azure Embedding API error: Http Status {response.status_code} "
                f"{response.reason_phrase}, Response: {body}",
                code="embedding.api_error",
            )
        try:
            parsed = AzureOpenAIEmbedResponse.model_validate_json(body)
...
async def new_azure_openai_embedder(
    *,
    api_key: str,
    base_url: str,
    model_name: str,
    ...
    """Build an Azure OpenAI embedder (``NewAzureOpenAIEmbedder``).

    The Azure resource endpoint (``base_url``) and the deployment name
    (``model_name``) are both required; ``api_version`` defaults to the
    upstream ``2024-10-21``.
    """
    if base_url == "":
        raise ValidationError(
            code="embedding.azure_base_url_required",
            message="Azure resource endpoint (base URL) is required",
        )
    if model_name == "":
        raise ValidationError(
            code="embedding.model_name_required",
            message="deployment name (model name) is required",
        )
    if api_version == "":
...
__all__ = ["AzureOpenAIEmbedder", "new_azure_openai_embedder"]
