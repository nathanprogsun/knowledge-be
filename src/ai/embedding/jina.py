"""Jina AI embedding provider (jina.go).

Implements text vectorization over the Jina AI ``/embeddings`` endpoint.
The wire shape is OpenAI-compatible except that Jina does not support
``truncate_prompt_tokens`` — it uses a boolean ``truncate`` flag instead,
which is always enabled (``truncate_prompt_tokens`` is accepted by the
constructor for signature symmetry but never sent).
...
class JinaEmbedRequest(BaseModel):
    """Jina ``/embeddings`` request body (``JinaEmbedRequest``).

    ``truncate`` is a boolean (always on) rather than the OpenAI integer
...
class JinaEmbedResponse(BaseModel):
    """``/embeddings`` response body (``JinaEmbedResponse``)."""
...
class JinaEmbedder:
    """Jina AI text vectorizer (``JinaEmbedder``)."""
    ...
    async def batch_embed(self, ctx: base.Context, texts: list[str]) -> list[list[float]]:
        ...
        try:
            parsed = JinaEmbedResponse.model_validate_json(response.text)
...
async def new_jina_embedder(
    *,
    ...
    """Build a Jina embedder (``NewJinaEmbedder``)."""
    if base_url == "":
...
__all__ = ["JinaEmbedder", "new_jina_embedder"]
