"""NVIDIA NIM embedding provider (nvidia.go).

Implements text vectorization over the NVIDIA ``/embeddings`` endpoint.
The wire shape is OpenAI-compatible plus an ``input_type`` field that
distinguishes query vectors from passage vectors: the upstream reads the
``types.EmbedQueryContextKey`` context marker, defaulting to ``passage``
and switching to ``query`` for interactive query embedding. The Python
``Context`` protocol does not carry that key, so the marker is read
defensively via ``getattr(ctx, "embed_query", False)``; the standard
``TaskContext`` leaves it off and passage is sent.
...
_EMBED_RETRIES = 3
_INPUT_TYPE_PASSAGE = "passage"
_INPUT_TYPE_QUERY = "query"


class NvidiaEmbedRequest(BaseModel):
    """NVIDIA ``/embeddings`` request body (``NvidiaEmbedRequest``)."""
...
class NvidiaEmbedResponse(BaseModel):
    """``/embeddings`` response body (``NvidiaEmbedResponse``)."""
...
class NvidiaEmbedder:
    """NVIDIA text vectorizer (``NvidiaEmbedder``)."""
    ...
    async def batch_embed(self, ctx: base.Context, texts: list[str]) -> list[list[float]]:
        request = NvidiaEmbedRequest(
            model=self._model_name,
            input=list(texts),
            encoding_format="float",
            input_type=(
                _INPUT_TYPE_QUERY if getattr(ctx, "embed_query", False) else _INPUT_TYPE_PASSAGE
            ),
        )
        ...
        try:
            parsed = NvidiaEmbedResponse.model_validate_json(response.text)
...
async def new_nvidia_embedder(
    *,
    ...
    """Build an NVIDIA embedder (``NewNvidiaEmbedder``)."""
    if base_url == "":
...
__all__ = ["NvidiaEmbedder", "new_nvidia_embedder"]
