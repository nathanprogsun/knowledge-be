"""Managed-cloud embedding provider (cloud.go).

Implements text vectorization over the the kb ``/api/v1/embeddings``
...
_DEFAULT_BASE_URL = "https://kb.weixin.qq.com"
_EMBED_PATH = "/api/v1/embeddings"
_DEFAULT_TIMEOUT_SECONDS = 60.0


class CloudEmbedRequest(BaseModel):
    """Cloud ``/embeddings`` request body (``cloudEmbedRequest``)."""
...
class CloudEmbedResponse(BaseModel):
    """Cloud ``/embeddings`` response body (``cloudEmbedResponse``)."""
...
class CloudEmbedder:
    """Managed-cloud text vectorizer (``CloudEmbedder``)."""
    ...
    async def batch_embed(self, ctx: base.Context, texts: list[str]) -> list[list[float]]:
        ...
        try:
            ...
            raise AIProviderError(
                f"kb embedder: do request: {exc}",
                code="embedding.request_failed",
            ) from exc
        body = response.text
        if response.status_code != 200:
            raise AIProviderError(
                f"kb embedder: status {response.status_code}: {body}",
                code="embedding.api_error",
            )
        try:
            parsed = CloudEmbedResponse.model_validate_json(body)
        except ValueError as exc:
            raise AIProviderError(
                f"kb embedder: unmarshal: {exc}",
    ...
    async def batch_embed_with_pool(
        ...
        # The kb embedder owns no pooler; kb embeds directly.
...
async def new_cloud_embedder(
    config: base.Config,
    *,
    timeout: float = _DEFAULT_TIMEOUT_SECONDS,
    transport: httpx.AsyncBaseTransport | None = None,
) -> CloudEmbedder:
    """Build a kb embedder (``NewCloudEmbedder``).
    ...
    if config.app_id == "":
        raise ValidationError(
            code="embedding.cloud_app_id_required",
            message="Cloud embedder: AppID is required",
        )
    if config.app_secret == "":
        raise ValidationError(
            code="embedding.cloud_app_secret_required",
            message="Cloud embedder: AppSecret is required",
        )
...
__all__ = ["CloudEmbedder", "new_cloud_embedder"]
