...
``CloudReranker`` posts the rerank payload to the kb
``/api/v1/rerank`` path with the app-id / app-secret signed ``X-*``
headers. The remote model name may be overridden through the
``remote_model_name`` extra config entry. ``new_cloud_reranker``
requires the kb credentials and applies the SSRF gate.
"""
...
# Path appended to the configured kb base URL.
_CLOUD_RERANK_PATH = "/api/v1/rerank"


class _CloudRankResult(BaseModel):
    """One ranked document from the kb ``results`` array."""
...
class _CloudResponse(BaseModel):
    """Decoded cloud rerank response body."""
...
class CloudReranker:
    """Managed-cloud reranker (upstream ``CloudReranker``)."""
    ...
    async def rerank(self, query: str, documents: list[str]) -> list[RankResult]:
        ...
        if response.status_code != 200:
            raise ExternalServiceError(
                ...
                message=(f"cloud reranker: status {response.status_code}: {response.text}"),
            )
        try:
            body = response.json()
            ...
            raise ExternalServiceError(
                ...
                message=f"cloud reranker: failed to decode response JSON: {exc}",
            ) from exc
        try:
            ...
            raise ExternalServiceError(
                code="rerank.cloud_invalid_response",
                message=f"cloud reranker: failed to parse response: {exc}",
            ) from exc
...
async def new_cloud_reranker(
    config: base.RerankerConfig,
    *,
    client: httpx.AsyncClient | None = None,
) -> base.Reranker:
    """Construct a ``CloudReranker`` (credentials + SSRF gate)."""
    if not config.app_id:
        raise ValidationError(
            code="rerank.cloud_app_id_required",
            message="Cloud reranker: app id is required",
        )
    if not config.app_secret:
        raise ValidationError(
            code="rerank.cloud_app_secret_required",
            message="Cloud reranker: app secret is required",
        )
...
__all__ = ["CloudReranker", "new_cloud_reranker"]
