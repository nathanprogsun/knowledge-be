...
Maps the upstream WeKnoraCloud VLM backend: posts the chat payload to
...
class WeKnoraCloudVLM:
    ...
    async def predict(self, img_bytes: list[bytes], prompt: str) -> str:
        ...
        if response.status_code != 200:
            raise AIProviderError(
                f"managed cloud VLM returned status {response.status_code}: "
                f"{response.text[:200]}",
                code="vlm.cloud_request_failed",
            )
        try:
...
async def new_weknoracloud_vlm(
    *,
    model_name: str,
    model_id: str,
    base_url: str,
