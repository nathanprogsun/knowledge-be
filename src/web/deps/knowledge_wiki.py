...
from src.core.knowledge.wiki.issues import WikiPageIssueRepository
...
def get_wiki_issue_repository(session: SessionDep) -> WikiPageIssueRepository:
    """Build a per-request issue repository on the shared session."""
    return build_wiki_issue_repository(session)
...
WikiIssueRepoDep = Annotated[WikiPageIssueRepository, Depends(get_wiki_issue_repository)]
